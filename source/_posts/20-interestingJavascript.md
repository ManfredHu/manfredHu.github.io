---
title: Interesting JavaScript
date: 2016-07-07 22:43:02
tags:
    - JavaScript
    - 随笔
categories:
...
JavaScript中`document.getElementById`的返回值的类型和`document.getElementsByTagName`的返回值类型分别是？
...
```
正确答案：A，B，C
解析：只有CSS可以用`@import`方法，题目其实可以换成CSS，则全部正确
...
如题所示，`test`方法如果参数为`null`或者不屑，则默认返回true
...
## 19.getPrototypeOf根据_proto_获取类型
...
你可以狠狠戳[这里](http://www.manfredhu.com/2016/03/27/13-proto-prototype-constructor/)去看我以前写的[15条规则解析JavaScript对象布局](http://www.manfredhu.com/2016/03/27/13-proto-prototype-constructor/)
...
```
答案；false
解析：min为Infinity，b为-Infinity，结果是`min > max`
...
第三个在第二步的基础上执行了匿名函数，最后再输出一个bbb，最后因为执行函数没有返回值，返回了`undefined`
